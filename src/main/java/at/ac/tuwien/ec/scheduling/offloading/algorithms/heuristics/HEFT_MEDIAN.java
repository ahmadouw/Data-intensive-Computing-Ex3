...
public class HEFT_MEDIAN extends OffloadScheduler { //extends the OffloadScheduler
	
	public HEFT_MEDIAN(MobileApplication A, MobileCloudInfrastructure I) {
	...
	public HEFT_MEDIAN(Tuple2<MobileApplication,MobileCloudInfrastructure> t) { 
		super();
	...
	private double upRank(MobileSoftwareComponent msc, DirectedAcyclicGraph<MobileSoftwareComponent, ComponentLink> dag,
		...
		if(!msc.isVisited()) //if component has not yet been visited
		{
			...
			Collections.sort(f1); //sort => needed for next step: calculation of median 
			double f1_func = 0;
			if (f1.size() % 2 == 1)
				f1_func = f1.get((f1.size() + 1) / 2 - 1); //f1 = MEDIAN
			else {
				double lower1 = f1.get(f1.size() / 2 - 1);
				double upper1 = f1.get(f1.size() / 2);
				f1_func =  (lower1 + upper1) / 2.0; //f1 = MEDIAN
			}
			...
			{ // if some additional dependent node exist (of the DAG)
				...
				{ // => i.e. all immediate succressors are investigated!
					...
					if(neigh.getTarget().isOffloadable())  // if the successor can be offloaded (else the communication cost is 0!):
					{
						...
						Collections.sort(f2); //sort => needed for next step: calculation of median 
						if (f2.size() % 2 == 1)
							f2_func = f2.get((f2.size() + 1) / 2 - 1); //f2 = MEDIAN
						else {
							double lower2 = f2.get(f1.size() / 2 - 1);
							double upper2 = f2.get(f1.size() / 2);
							f2_func =  (lower2 + upper2) / 2.0; //f2 = MEDIAN
						}
