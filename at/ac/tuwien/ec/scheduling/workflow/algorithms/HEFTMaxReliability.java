...
public class HEFTMaxReliability extends WorkflowScheduler {
	
	
	public HEFTMaxReliability(MobileApplication A, MobileCloudInfrastructure I) {
		super();
	...
	public HEFTMaxReliability(Tuple2<MobileApplication,MobileCloudInfrastructure> t) {
		super();
	...
	public ArrayList<? extends Scheduling> findScheduling() {
		...
		while((currTask = tasks.poll())!=null)
		{
			...
			else if(currTask.getId().startsWith("BARRIER"))
			{
				...
				{
					double currRel = 0.0;
					if(cn instanceof EdgeNode)
						currRel = ((EdgeNode)cn).getAvailabilityAt(0);
					if(cn instanceof CloudDataCenter)
						currRel = ((CloudDataCenter)cn).getAvailabilityAt(0);
					if( currRel > maxRel &&
